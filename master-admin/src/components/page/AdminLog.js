...
function loadData(listRow = 10, listCount = 10, search) {
    ...
    let q = 'query { inquiries (where:"{\\"type\\":\\"00\\", \\"admin\\": null, \\"OR\\": [{\\"title_contains\\": \\"' + search + '\\"}, {\\"user\\": { \\"name_contains\\": \\"' + search + '\\"} } ] }" orderBy: "createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount inquiries { id title contents createdAt reply { id title contents } user { id name orders { status payMethod } } } } }'
            ...
            .then(function (response) {
                ...
                if (copyInquiry.reply.id !== undefined) {
                    ...
                    axios({
                        ...
                        "method": "post",
                        "data": {
                            query: 'mutation { updateInquiry (id: "' + copyInquiry.id + '", replyId: "' + newReplyId + '", status:"1") { id } }'
                        }
                    })
                                    ...
                                    inquiries.map(row => (
                                        <StyledTableRow key={ row.id }>
                                            ...
                                            <StyledTableCell align="right">
                                                {
                                                    row.user !== null &&
                                                        numeral(getOrderCount(row.user.orders)).format('0,0')
                                                }
                                                </StyledTableCell>
