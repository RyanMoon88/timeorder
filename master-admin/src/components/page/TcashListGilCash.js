...
function loadData(search = "", status = "", listRow = 10, listCount = 10, start, end) {
    ...
    switch (status) {
        case "00":      // 충전
            // q = 'query {' +
            //     ' tcashes (where: "{\\"OR\\": [{\\"AND\\": [{\\"type\\": \\"TC00\\"}, {\\"payMethod\\": \\"CARD\\"}] }], \\"user\\": {\\"name_contains\\":\\"' + search + '\\"}, \\"pgType_contains\\":\\"GIL_\\", \\"createdAt_gte\\":\\"' + momentStartDate.utc().format() + '\\", \\"createdAt_lt\\":\\"' + momentEndDate.utc().format() + '\\"}" orderBy:"createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount tcashes { id oid createdAt user { id name orders { id } } type payMethod memo price tcash totalCount order { store { name } items { product { name } } } } } }';
            type = "charge";
            break;
        case "01":      // 사용
            // q = 'query {' +
            //     ' tcashes (where: "{\\"OR\\": [{\\"AND\\": [{\\"type\\":\\"TC01\\"}]}], \\"user\\": {\\"name_contains\\":\\"' + search + '\\"}, \\"pgType_contains\\":\\"GIL_\\", \\"createdAt_gte\\":\\"' + momentStartDate.utc().format() + '\\", \\"createdAt_lt\\":\\"' + momentEndDate.utc().format() + '\\"}" orderBy:"createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount tcashes { id oid createdAt user { id name } type payMethod memo price tcash totalCount order { store { name } items { product { name } } } } } }';
            type = "use";
            break;
        case "02":      // 거절
            // q = 'query {' +
            //     ' tcashes (where: "{\\"OR\\": [{\\"AND\\": [{\\"type\\":\\"TC00\\"}, {\\"payMethod\\": \\"REFUND\\"}]}], \\"user\\": {\\"name_contains\\":\\"' + search + '\\"}, \\"pgType_contains\\":\\"GIL_\\", \\"createdAt_gte\\":\\"' + momentStartDate.utc().format() + '\\", \\"createdAt_lt\\":\\"' + momentEndDate.utc().format() + '\\"}" orderBy:"createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount tcashes { id oid createdAt user { id name } type payMethod memo price tcash totalCount order { store { name } items { product { name } } } } } }';
            type = "reject";
            break;
        case "03":      // 차감
            // q = 'query {' +
            //     ' tcashes (where: "{\\"OR\\": [{\\"AND\\": [{\\"type\\":\\"TC00\\"}, {\\"payMethod\\": \\"CHARGE\\"}, {\\"tcash_lt\\": 0}]}], \\"user\\": {\\"name_contains\\":\\"' + search + '\\"}, \\"pgType_contains\\":\\"GIL_\\", \\"createdAt_gte\\":\\"' + momentStartDate.utc().format() + '\\", \\"createdAt_lt\\":\\"' + momentEndDate.utc().format() + '\\"}" orderBy:"createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount tcashes { id oid createdAt user { id name } type payMethod memo price tcash totalCount order { store { name } items { product { name } } } } } }';
            type = "deducation";
            break;
        case "04":      // 퀴즈정답
            // q = 'query {' +
            //     ' tcashes (where: "{\\"OR\\": [{\\"AND\\": [{\\"type\\":\\"TC00\\"}, {\\"payMethod\\": \\"TIMEQUIZ\\"}]}], \\"user\\": {\\"name_contains\\":\\"' + search + '\\"}, \\"pgType_contains\\":\\"GIL_\\", \\"createdAt_gte\\":\\"' + momentStartDate.utc().format() + '\\", \\"createdAt_lt\\":\\"' + momentEndDate.utc().format() + '\\"}" orderBy:"createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount tcashes { id oid createdAt user { id name } type payMethod memo price tcash totalCount order { store { name } items { product { name } } } } } }';
            type = "quiz";
            break;
        case "05":      // 친구추천
            // q = 'query {' +
            //     ' tcashes (where: "{\\"OR\\": [{\\"AND\\": [{\\"type\\":\\"TC00\\"}, {\\"payMethod\\": \\"USER_RECOMMEND\\"}]}], \\"user\\": {\\"name_contains\\":\\"' + search + '\\"}, \\"pgType_contains\\":\\"GIL_\\", \\"createdAt_gte\\":\\"' + momentStartDate.utc().format() + '\\", \\"createdAt_lt\\":\\"' + momentEndDate.utc().format() + '\\"}" orderBy:"createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount tcashes { id oid createdAt user { id name } type payMethod memo price tcash totalCount order { store { name } items { product { name } } } } } }';
            type = "friend";
            break;
        default:
            // q = 'query {' +
            //     ' tcashes (where: "{\\"OR\\": [{\\"AND\\": [{\\"type\\": \\"TC00\\"}, {\\"payMethod_not\\": \\"00\\"}]}, {\\"type\\":\\"TC01\\"}], \\"user\\": {\\"name_contains\\":\\"' + search + '\\"}, \\"pgType_contains\\":\\"GIL_\\", \\"createdAt_gte\\":\\"' + momentStartDate.utc().format() + '\\", \\"createdAt_lt\\":\\"' + momentEndDate.utc().format() + '\\"}" orderBy:"createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount tcashes { id oid createdAt user { id name } type payMethod memo price tcash totalCount order { store { name } items { product { name } } } } } }';
            type = "all";
            break;
    }
    ...
    return new Promise((resolve, reject) => {
        axios({
            "url": API_HOST + "/master/loadGilCashList",
            "method": "post",
            "params": {
                // startDate:(momentStartDate.format("YYMMDD") + "000000"),
                // endDate: (momentEndDate.format("YYMMDD") + "000000"),
