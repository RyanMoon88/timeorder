...
function loadData(search = "", listRow = 10, listCount = 10, start, end, deviceTab, statusTab) {
    ...
    console.log(device);
    console.log(strDeviceNull);
    console.log(status);
    console.log(momentStartDate.format("YYMMDDHHmmss"));
    let orderListParam = {
        s2Date:momentStartDate.format("YYMMDDHHmmss"),
        e2Date:momentEndDate.format("YYMMDDHHmmss"),
        paging:listRow,
        pageSize:listCount,
        statusList:status,
        deviceList:device,
    ...
    return new Promise((resolve, reject) => {
        axios({
            "url": API_HOST + "/master/loadOrderListByMaster",
            "method": "post",
            "data":orderListParam
        })
...
function setExtraInfo(row) {
    let result = "";
    try {
        result = JSON.parse(row.extraInfo).memo;
    }
    catch (e) {
...
function execExcelData(startDate, endDate) {
    ...
    window.open(API_CAL_URL + "/master/execExcelDownloadByPGL?status=LIST_ORDER&startDate=" + start + "&endDate=" + end, '_blank');
}
...
const OrderList = ({ match }) => {
    ...
    function clickPagination(offset) {
        ...
        setBackdrop(true);
        loadData(search, offset , listCount, startDate, endDate, deviceTabValue, statusTabValue)
        .then((response) => {
            setData(response);
            setBackdrop(false);
        });
                        ...
                        <Table className={classes.table} aria-label="customized table">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <StyledTableCell align="center" style={{ width:'90px' }}>주문기기<br/>주문자</StyledTableCell>
                                ...
                                }
                                {
                                    orders.map(row => (
                                        <StyledTableRow key={ row.id }>
                                            <StyledTableCell component="th" scope="row" align="center"><small>{ row.oid }</small><br/>{ row.authDate }</StyledTableCell>
                                            <StyledTableCell>
                                                <div>{ row.device }</div>
                                                {
                                                    row.user !== null && 
                                            ...
                                            <StyledTableCell align="right">{ numeral(setCouponDiscount(row)).format('0,0') }</StyledTableCell>
