...
import { Box, Typography, Grid, Container, Button, TextField } from '@material-ui/core';
...
function loadData(listRow = 10, listCount = 10) {
    let q = 'query { notices (where:"{\\"OR\\":[{\\"type\\":1},{\\"type\\":2}]}"orderBy: "createdAt_DESC", skip:' + listRow + ', first:' + listCount + ') { totalCount notices { id title contents createdAt updatedAt } } }'

    return new Promise((resolve, reject) => {
        axios({
            "url": API_HOST + "/master/loadNoticeListByMaster",
            "method": "post",
            "params": {
                page: listRow,
                pageSize: listCount,
